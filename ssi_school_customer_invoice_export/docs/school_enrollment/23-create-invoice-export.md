...
> **Module:** ssi*school_customer_invoice_export\
> **Model:** `school_enrollment`\
> **Extends:** ssi_school — model `school_enrollment`\
...
- **Record:** At least one Payment Term has an unpaid (**Open**) Customer Invoice that
  already carries a posted journal entry.
...
- A new **Customer Invoice Export** document is created in **Draft** status, linked to
  the unpaid invoice move(s) collected from this enrollment's Payment Terms.
...
> **Note:** This wizard can also be run in bulk over several enrollments selected from
> the **School > Student Activities > Enrollments** list view, via **Action > Create
> Invoice Export** — every selected enrollment must individually pass the
> `create_invoice_export_ok` policy, and every enrollment's unpaid invoices are combined
