...
#       failed the 2026-09-04 production deploy. The fix replays the
#       compute through the same ``env.add_to_compute`` + ``flush``
#       idiom the ORM itself uses for a normal recompute, which writes
#       at the field level under ``env.protecting`` and never calls
#       ``write()``.
...
def migrate(env, version):
    ...
    ``school_enrollment_payment_term`` table via
