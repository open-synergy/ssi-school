...
class SchoolAdmissionTestCreateAdmission(models.TransientModel):
    ...
    a school_admission_test, letting the operator override the
    ...
    def action_create_admission(self):
        """Create the admission from the admission test, or reopen it.

        Idempotent: if a ``school_admission`` already links to
        ``admission_test_id``, that existing record is reopened and no
