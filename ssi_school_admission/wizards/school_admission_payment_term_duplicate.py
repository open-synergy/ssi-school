...
class SchoolAdmissionPaymentTermWizardDuplicate(models.TransientModel):
    """
    Wizard that duplicates a school_admission_payment_term record.
    Copies every detail line (detail_ids) from the source term to a new term
    on the same admission, while only the header fields (name, sequence,
    date_invoice, date_due) are taken from the wizard input.
    """

    _name = "school_admission_payment_term.wizard_duplicate"
    _description = "Duplicate School Admission Payment Term"
    ...
    sequence = fields.Integer(
        ...
        default=5,
        help=(
            "Order of the new payment term within the admission. "
    ...
    def default_get(self, fields_list):
        ...
        if active_id:
            term = self.env["school_admission_payment_term"].browse(active_id)
            res["term_id"] = term.id
