# Copy Payment Terms — Admission

> **Module:** `ssi_school_admission`\
> **Model:** `school_admission`\
> **Menu:** School > Admission > Admissions\
> **Actor:** user in group _Admission — User_ (`school_admission_user_group`)\
> **Requires:** `01-create`

This action has no button on the form. It is a contextual action, reached from the
**Action** menu of the Admissions **list** view, and it acts on every admission ticked
there — so it is not a step in this model's own lifecycle and no `State:` is declared
above.

## Pre-Condition

- **Record:** Every target admission is in status **Draft**. A target in any other
  status is rejected by name when the wizard is confirmed, and nothing is copied.
- **Record:** Every target admission has the same **Academic Term**, **School**, and
  **Grade** as the source admission. Mismatched targets are rejected by name, and
  nothing is copied.
- **Data:** A source admission exists and already carries the Payment Terms to copy. The
  source is chosen inside the wizard and may be in any status — only the targets are
  restricted.
- **Config:** An active `policy.template` for this model grants `copy_payment_term_ok`
  for state `draft` to the actor's group.
- **Access:** User is in group _Admission — User_ (`school_admission_user_group`).

## Flow

1. Open the **School > Admission > Admissions** menu.
2. In the list view, tick the checkbox of every admission that will **receive** the
   payment terms.
3. Click **Action** > **Copy Payment Terms**.
4. In the wizard, the ticked records are shown read-only in **Target Admissions**. To
   change the selection, close the wizard and tick a different set.
5. Fill in the fields:
   - **Source Admission** _(required)_: the admission whose Payment Terms (and their
...
- Every target admission carries a copy of the source admission's Payment Terms,
...
- The copies are **uninvoiced**: the link to the source's customer invoice and invoice
  ...
  the target's own **Create Due Invoice** (see `15-create-due-invoice.md`).
...
- If any target fails a check, **no** target is changed — every target is validated
  before the first copy is made — and the wizard reports the offending admissions by
  name.
