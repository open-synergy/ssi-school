# Reload Template Policy — Admission
...
- **Policy Template** is recomputed and re-assigned to the highest-sequence
  ...
  `restart_approval_ok`, `addendum_ok`, `create_invoice_ok`, `create_enrollment_ok`,
  etc.) are granted, without changing the record's status.
