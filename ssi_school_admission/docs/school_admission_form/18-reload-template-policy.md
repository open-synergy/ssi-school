# Reload Template Policy — Admission Form
...
- **Policy Template** is recomputed and re-assigned to the highest-sequence
  ...
  `restart_approval_ok`, `create_admission_ok`, `create_admission_test_ok`, etc.) are
  granted, without changing the record's status.
