...
class SchoolAdmissionWizardCreateInvoiceExport(models.TransientModel):
    """
    Wizard that collects the unpaid customer invoices generated from the
    payment terms of one or more selected admissions and exports them
    into a draft ``customer_invoice_export`` document, ready to be
    confirmed. Every selected admission must be allowed by the
    create_invoice_export_ok policy (state and group, configurable via
    Policy Template). All admissions are validated, and the created
    document must have at least one summary row, before the wizard
    completes.
    """

    _name = "school_admission.wizard_create_invoice_export"
    _description = "Create Invoice Export for Admission Payment Terms"

    admission_ids = fields.Many2many(
        string="Admissions",
        comodel_name="school_admission",
        relation="rel_school_admission_create_invoice_export",
        column1="wizard_id",
        column2="admission_id",
        readonly=True,
        help="Admissions whose unpaid invoices will be exported.",
    )
    ...
    def default_get(self, fields_list):
        """Pre-fill ``admission_ids`` from the calling window action.
    ...
    def _create_invoice_export(self):
        ...
        Validates the selected admissions, collects the unpaid invoice
        moves from all of them via
        ``school_admission._get_exportable_move``, creates the export
        document with those moves in ``source_move_ids``, populates it,
        and verifies at least one summary row was produced.

        :raises UserError: no admission selected, an admission is not
        ...
        for admission in self.admission_ids:
            moves |= (
                admission._get_exportable_move()
    ...
    def _check_admissions(self):
        """Validate the selected admissions before exporting.

        :raises UserError: no admission selected, or one or more
            selected admissions are not allowed by the create invoice
            export policy
        """
        self.ensure_one()
        if not self.admission_ids:
            error_message = (
                _(
                    """
Context: Create invoice export for admissions
Database ID: %s
...
"""
        ...
        problems = []
        for admission in self.admission_ids:
            if not admission.create_invoice_export_ok:
                problems.append(
...
Context: Create invoice export for admissions
Database ID: %s
Problem: The following admission(s) are not allowed to create invoice export:
%s
Solution: Deselect the listed admission(s), or make sure they are open/done and allowed
...
Context: Create invoice export for admissions
Database ID: %s
Problem: No exportable move found on the selected admission(s)
Solution: Select admission(s) that have at least one payment term with an unpaid \
